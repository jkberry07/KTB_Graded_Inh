...
mGABA_raw = randn(mitralNum,granuleNum)*mGABAmean/10 + mGABAmean; %GABA conductance
...
%If want to record mVolt and gVolt, add to simulator
%function below

[mSpikeTrain, gSpikes, LFP_AMPA, LFP_NMDA, LFP_GABA, LFP_GABA_ton]...
    = simulator(network, mParam, gParam,...
...
fname_1 = sprintf('LFP50_GPFI_mGABAlvl%g_%ggloms_inputlvl%g_tilvl%g_trial%g.mat',...
    mGABAindx,odorGlomNum,inputindx,tiindx,trial);
save(fname_1,'mSpikeTrain','gSpikes', 'odorGloms','LFP_AMPA', 'LFP_GABA', 'LFP_GABA_ton',...
    'LFP_NMDA','-v7.3');
...
function [mSpikeTrain, gSpikes, LFP_AMPA, LFP_NMDA, LFP_GABA,...
...
%     gVolt = zeros(granuleNumRecord, length(tspan));
    ...
    sGABA_tot = zeros(mitralNum, granuleNum);
...
%         end
        ...
        mIntegG(:,gFired) = sGABA_tot(:,gFired) + network(:,gFired)*W.*(1-sGABA_tot(:,gFired));
        
        
        % calculate external and synaptic currents 
        [mI, gI, mIntegG,sGABA_tot,mIntegA,mIntegN,mX,gIntegA,gIntegN, gX, LFP_AMPA(t),...
            LFP_NMDA(t), LFP_GABA(t), LFP_GABA_ton(t)] = current(mV, gV, gFired, mag,...
...
function [mI, gI, mIG, sGtot, mIA,mIN,Xm,gIA,gIN, Xg, LFP_A, LFP_N, LFP_G, LFP_GGC] = current(mV, gV, gFired, Mg,...
          mIntegG, mIntegA, mIntegN, mX, gIntegA, gIntegN, gX,...
          mGABA_raw, mGABA, tauG_m, mAMPA, tauA_m, mNMDA, tauNr_m, tauNd_m,...
          gAMPA, tauA_g, gNMDA, tauNr_g, tauNd_g, TS,...
          distance_3D, mitralNum, granuleNum, distance_GC,tilvl)
        ...
        BV = 1./(1+exp(-0.062*gDiff)*Mg/3.57); % since Ee = 0, gDiff = gV matrix;

        gNotFired = setdiff(1:size(mIntegG,2),gFired); %find indices of GCs that didn't fire
        sGraded = zeros(size(mIntegG));
        sGraded(:,gFired) = 0;
        sGraded(:,gNotFired) = gIntegN(:,gNotFired) .* BV(:,gNotFired);
        sGtot = mIntegG + sGraded; %In Osinski, GABA release followed NMDA current,
                                %so here the gating variable is however
                                %much the NMDA gate is open plus GABA
                                %release due to firing
        sGtot(sGtot>1) = 1; %make sure it doesn't go above 1

        % calculate current matrices 
        Ia_mat = gDiff .* gAMPA .* gIntegA;
        In_mat = gDiff .* gNMDA .* gIntegN .* BV;
        Ig_mat = mDiff .* mGABA .* sGtot;
        Ig_mat_raw = mDiff .* mGABA_raw .* sGtot;
